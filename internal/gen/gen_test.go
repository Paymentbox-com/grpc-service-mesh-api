...
func TestGenerate_OnlyRequestedLanguage(t *testing.T) {
	...
	outs, err := Generate(set, []Lang{Ruby})
	if err != nil {
		t.Fatal(err)
	}
...
func TestGenerate_GoPackageErrorReportedOnceAcrossFiles(t *testing.T) {
	...
	_, err := Generate(set, []Lang{Go, Ruby})
...
func TestWrite_PutsFilesUnderOutAndLanguage(t *testing.T) {
	out := t.TempDir()
	paths, err := Write(out, []Output{
	...
	if len(paths) != 2 || paths[0] != filepath.Join(out, "go", "pbx", "pbx.grpcmesh.go") || paths[1] != filepath.Join(out, "ruby", "service_maps.rb") {
